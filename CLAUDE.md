...
- The model does REASONING only. `web_search` covers news/qualitative facts,
  not price fetching. Web search runs on **OpenAI's built-in `web_search`**
  (Responses API, gpt-4.1-mini) — budget new search-using features against
  OpenAI's per-search fee, not just the per-request cent caps.
...
- **Single-provider AI: OpenAI only, `gpt-4.1-mini` for EVERY function**, all
  behind ONE adapter at `src/lib/openai.ts` — never import the `openai` package
  (or any other model SDK) directly anywhere else. NO Anthropic/Claude, NO
  OpenRouter/DeepSeek. The adapter presents the same Anthropic-shaped surface
  the codebase always used and translates it ⇄ the OpenAI Responses API. Every
  call — reasoning AND web-search — runs on OpenAI. Web search uses OpenAI's
  built-in `web_search` tool; the adapter passes
  `include:["web_search_call.action.sources"]` and reconstructs Anthropic-shaped
  `server_tool_use` + `web_search_tool_result` blocks from OpenAI's output, so
  the provenance code (sweep/picker/regime/mosaic/earnings/analyst/headline)
  validates cited source URLs exactly as before. Agents pass the ids from
  `MODELS` (all `gpt-4.1-mini`). Only `OPENAI_API_KEY` is required
  (`ANTHROPIC_API_KEY` / `OPENROUTER_API_KEY` are no longer used). Never invent
  a model ID.
...
3. Judge synthesizes verdict + conviction + bull/bear lines, carrying sources
   through for the collapsed Sources accordion. Every judge — base Council,
   comparative, sector — runs on **gpt-4.1-mini**, like the whole app.
   (Match `MODELS` in `openai.ts`.)
