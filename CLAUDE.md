...
- **No annual billing.** `pro_annual` stays in the code only so the webhook keeps
  ...
  `STRIPE_PRICE_PRO_MONTHLY` at $8/mo.
...
- **Paid data APIs: ONE budgeted exception** (founder decision 2026-06-11,
  ...
  **FMP Starter (~$19–29/mo)**. Its adapter ships dormant in
  ...
  Alpha Vantage, no NewsAPI, no Marketaux); the OpenAI API is the only other
  paid external API.
...
- The model does REASONING only. `web_search` covers news/qualitative facts,
  not price fetching. Web search runs on **Anthropic** (~1¢ per search, $10 /
  1000) and respects the `max_uses` caps in the tool defs — budget new
  search-using features against that, not just the per-request cent caps.
- Headlines/Dashboard content is generated by scheduled gpt-4.1-mini jobs, cached in
...
- **Dual-provider AI**, all behind ONE adapter at `src/lib/openai.ts` — never
  import the `openai` or `@anthropic-ai/sdk` packages directly anywhere else.
  The adapter routes per-call: any call that includes a web_search tool runs on
  **Anthropic/Claude** (its web_search is ~1¢/call vs OpenAI's ~2.5¢, and the
  results are only readable by Claude, so search + the reasoning over it are
  inseparable); every pure-reasoning call runs on **OpenAI** via the Responses
  API (the only OpenAI surface that runs built-in web_search alongside function
  tools — kept for parity though search now goes to Claude). Agents pass the
  OpenAI ids from `MODELS`; the adapter translates them to the Claude tier on
  the search path (`gpt-4.1`→Sonnet 4.6, `gpt-4.1-mini`→Haiku 4.5). Tiering:
  `gpt-4.1-mini` for specialists, routing, sweep, scheduled content, base
  Council judge; `gpt-4.1` for the marquee comparative/sector judges, picker,
  CIO constructor, and Analyst chat. Search-using agents (sweep, headline,
  analyst, picker, regime, mosaic, feed) therefore execute on Claude; the
  pure-reasoning ones (specialists, judges, CIO, router) on OpenAI. Both
  `OPENAI_API_KEY` and `ANTHROPIC_API_KEY` are required. Never invent a model ID.
...
1. Sweep agent (gpt-4.1-mini + web_search) builds a FactSheet of sourced
...
3. Judge synthesizes verdict + conviction + bull/bear lines, carrying sources
   ...
   **gpt-4.1-mini** (cost-forced — the prompt is tight enough); only the marquee
   comparative/sector judges use gpt-4.1. (Match `MODELS` in `openai.ts`.)
