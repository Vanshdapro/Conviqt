...
- The model does REASONING only. `web_search` covers news/qualitative facts,
  not price fetching. OpenAI's `web_search` costs ~2.5¢ per call (~$25 / 1000 at
  "low" context) — budget new search-using features against that, not just the
  per-request cent caps. (Note: there is no hard per-call cap like the old
  `max_uses`; the search budget is governed by the prompts.)
...
- OpenAI only for AI, via the `openai` SDK's **Responses API** (the only OpenAI
  surface that runs built-in web_search alongside custom function tools). All
  calls go through the adapter in `src/lib/openai.ts`, which presents the same
  request/response shape the agents have always used — never import the `openai`
  SDK directly anywhere else. Use the pinned model IDs from `MODELS` in that
  file: `gpt-4.1-mini` for specialists, intent routing, sweep, scheduled
  content, and the base Council judge (the cheap, high-frequency work); `gpt-4.1`
  for the marquee comparative/sector judges, the stock picker, the CIO
  constructor, and the general Analyst chat. Never invent a model ID.
